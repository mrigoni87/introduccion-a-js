...
alert("mi nombre es " + nombre + " y mi edad: " + edad);
...
/*
    ...
    Es lo mismo con el código. Le podés dar una condición 'if' a una máquina para que haga 
    una decisión acerca de qué
    ...
    Necesitamos que la condición sea un Booleano, recordemos: TRUE o FALSE. 
...
/*
    ...
    Ahora es momento de
    presentar el siguiente set de operadores, los 'Operadores de comparación'. Son usados para 
    comparar valores:
    >   // Mayor que
    <   // Menor que
    <=  // Menor o igual que (se escribe en este orden, =< no existe)
    >=  //Mayor o igual que (se escribe en este orden, => es usado para otra cosa)
    === //Exactamente igual que (1 y "1" no son iguales, pero 1 es exactamente igual a 1 y "1" es 
    exactamente igual a  "1"). Este operador compara el valor y el TIPO del valor.
    ...
    También están los operadores '==' y '!=' que son muy similares a '===' y '!==' con una pequeña 
    variación,
    que los hacen que sean más propensos a errores, así que siempre que sea posible vamos a usar 
    '===' y '!=='.
...
if(operador === '+'){
    console.log(
        sumar(numeroCual1,numeroCual2)
        );
}else{
    console.log(
        restar(numeroCual1,numeroCual2)
        );
}
...
// TAREA: Cambiar el operador a '-', y fijarse que llame a la función 'restar' en vez de a la de 
//'sumar'.
...
/*
    ...
    Bueno, y qué pasa si tenemos 4 operaciones aritméticas en nuestra calculadora? Bueno, entonces 
    usamos una estructura así (pseudocódigo):
    if(condicion)
        //algo
    else if(condicion)
        //algo
...
if(operador === '+'){
    ...
    console.log(
        multiplicar(numeroCual1,numeroCual2)
        );
}
else if(operador==='/'){
    console.log(
        dividir(numeroCual1,numeroCual2)
        );
}
else{
    console.log(
...
// TAREA: usando el operador !, Intentá invertir una variable (de true a false, o de false a true) 
//e imprimí el
